...
public class TextArchitect
{
    ...
    {
        int midRaneg = preTextLength;
        int maxRange = midRaneg + 1;
        ...
        Color32[] vertexColor = textInfo.meshInfo[textInfo.characterInfo[0].materialReferenceIndex].colors32;
        float[] alpha = new float[textInfo.characterCount];

        while (true)
        {
            float fadeSpeed = ((hurryUp ? characterPerCycle * 5 : characterPerCycle) * speed) * 4f;

            for (int i = midRaneg; i < maxRange; i++)
            {
                TMP_CharacterInfo charInfo = textInfo.characterInfo[i];
                ...
                for (int v = 0; v < 4; v++)
                {
                    vertexColor[charInfo.vertexIndex + v].a = (byte)alpha[i];
                }
            ...
            if (alpha[maxRange - 1] > alphaThreshold || lastCharacterIsInvisible)
            {
                if (maxRange < textInfo.characterCount)
                {
                    maxRange++;
                }
                else if (alpha[maxRange - 1] >= 255 || lastCharacterIsInvisible)
                    break;
            }
